...
export default createCommand({
	...
	runs: (msg) => {
		...
		if (user?.user === msg.author)
			return sendMessage(
				msg.channelID,
				`You want to kiss yourself?!?! Please Mention a user to kiss!`
			);
		if (user) {
			const output = new MessageEmbed()
				.setTitle(`**Kiss 💋**`)
				.setDescription(
					`**${msg.author.username}** kissed **${user.user.username}!**`
				)
			...
			sendMessage(msg.channelID, 'that person is not in this guild');
		}
	},
});
